...
include "bigint_functions.circom";
...
template BigAddB(n, k) {
    assert(n <= 252);
    signal input a[k];
    signal input b[k];
    signal output out[k + 1];

    component unit0 = ModSumB(n);
    unit0.a <== a[0];
    unit0.b <== b[0];
    out[0] <== unit0.sum;
...
template BigMultNoCarryB(n, ma, mb, ka, kb) {
    assert(ma + mb <= 253);
    signal input a[ka];
    signal input b[kb];
...
template LongToShortNoEndCarryB(n, k) {
    assert(n <= 126);
    signal input in[k];
    signal output out[k+1];
    ...
    var split[k][3];
    for (var i = 0; i < k; i++) {
        split[i] = SplitThreeFnB(in[i], n, n, n);
    }
...
template BigMultB(n, k) {
    signal input a[k];
    signal input b[k];
    signal output out[2 * k];

    component mult = BigMultNoCarryB(n, n, n, k, k);
    for (var i = 0; i < k; i++) {
        mult.a[i] <== a[i];
        mult.b[i] <== b[i];
    }
    ...
    component longshort = LongToShortNoEndCarryB(n, 2 * k - 1);
...
template BigModB(n, k) {
    ...
    var longdiv[2][100] = long_divB(n, k, k, a, b);
    for (var i = 0; i < k; i++) {
        div[i] <-- longdiv[0][i];
        mod[i] <-- longdiv[1][i];
    }
...
template BigSubModPB(n, k){
    assert(n <= 252);
    signal input a[k];
    signal input b[k];
    signal input p[k];
    signal output out[k];
    component sub = BigSubB(n, k);
    for (var i = 0; i < k; i++){
        sub.a[i] <== a[i];
        sub.b[i] <== b[i];
    }
...
template BigMultModPB(n, k) {
    ...
    component big_mult = BigMultB(n, k);
    for (var i = 0; i < k; i++) {
        big_mult.a[i] <== a[i];
        big_mult.b[i] <== b[i];
    }
    component big_mod = BigModB(n, k);
    for (var i = 0; i < 2 * k; i++) {
...
template BigModInvB(n, k) {
    ...
    component mult = BigMultB(n, k);
    for (var i = 0; i < k; i++) {
        mult.a[i] <== in[i];
        mult.b[i] <== out[i];
    }
    component mod = BigModB(n, k);
    for (var i = 0; i < 2 * k; i++) {
        mod.a[i] <== mult.out[i];
    }
    for (var i = 0; i < k; i++) {
...
template CheckCarryToZeroB(n, m, k) {
    assert(k >= 2);
