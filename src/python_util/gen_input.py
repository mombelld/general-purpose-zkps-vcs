...
def decode_vc(vc_path):
    with open(vc_path, "r") as vc_file:
        ...
        for e in encoded_disclosures_list:
            decoded_disclosure =  base64.urlsafe_b64decode(e + '==').decode('utf-8')
...
def load_status_list(status_list_name):
    ...
    status_list_path = os.path.join(base_path, "..", "..", "status_list", status_list_name)

    status_list = b""
    with open(status_list_path, "r") as status_list_file:
        status_list = status_list_file.read()

    n_chunks = gen_status_list.TOTAL_SIZE // INT_SIZE + (1 if gen_status_list.TOTAL_SIZE % INT_SIZE != 0 else 0)
...
def main():
    ...
    input_data = {}
    proof_inputs_l, inputs_order_l, input_types_l, prefix_l = get_proof_inputs(os.path.join(proof_path, "proof_metadata.json"))

    input_data = add_auxiliary_input(os.path.join(proof_path, "auxiliary_inputs.json"), input_data)
    for i, credential_path in enumerate(credential_paths):
        ...
        claim_names, claim_values, hashed_claims = encode_claims(claims, inputs_order, input_types, date_offset)
        for inp in proof_inputs:
            if inp == "status_list_uri":
                input_data[f"{prefix}status_list"] = load_status_list(status_list_uri)
        ...
        if "cnf_jwk_x" in proof_inputs:
            input_data = hardware_binding(os.path.join(proof_path, "hb_keypair.json"), input_data, prefix)
